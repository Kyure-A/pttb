function getToday(delay: number = 0): string {
    const date: Date = new Date();
    date.setDate(date.getDate() + delay);
    return Utilities.formatDate(date, "JST", "M/dd");
}

function getDayOfWeek(delay: number = 0): string {
    const date: Date = new Date();
    date.setDate(date.getDate() + delay);
...
function getSheet(sheet_name: string): string[][] {
...
function genTitle(delay: number = 0): string {
...
function is_specialday(today: string): boolean {
    const sheet: GoogleAppsScript.Spreadsheet.Spreadsheet = SpreadsheetApp.getActiveSpreadsheet();
    if (sheet.getSheetByName(today) != null) return true;
    else return false;
}

function genFields(today: string, day_of_week: string): TimeTable[] {
    let value: string[][];
